#!/usr/bin/env python3
"""
Updated manager.py for VPS with git pull functionality
This script should be copied to /opt/logo-detection/manager.py on your VPS
"""
...
@app.route("/git/build", methods=["POST"])
def git_build():
    """Build Docker image from local git repository"""
    if not os.path.exists(GIT_REPO_DIR):
        return jsonify({
            "error": "Repository not found",
            "message": f"No repository at {GIT_REPO_DIR}. Use /git/pull to clone."
    ...
    # Build Docker image
    print("Building Docker image from repository...")
    results["build"] = run_command(f"cd {GIT_REPO_DIR} && docker build -t {DOCKER_IMAGE} .")
    
    if results["build"]["success"]:
        # Show image info
        results["image_info"] = run_command(f"docker images {DOCKER_IMAGE} --format 'Size: {{{{.Size}}}}, Created: {{{{.CreatedAt}}}}'")
        
        # Ask if should deploy
        auto_deploy = request.json.get("auto_deploy", False) if request.json else False
        if auto_deploy:
            deploy_result = deploy()
            results["deploy"] = deploy_result[0].json
    ...
    return jsonify({
        ...
        "message": "Build completed" if results["build"]["success"] else "Build failed",
        "results": results
    }), 200 if results["build"]["success"] else 500
